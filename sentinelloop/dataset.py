...
from .simulation import (
    account_baseline_inr,
    event_lifecycle_phase,
    simulate_attack,
...
def _ambient_events(accounts: list[str], rng: random.Random, n_rows: int, start: datetime) -> list[dict[str, Any]]:
    ...
    while len(rows) < n_rows:
        ...
        for seq in range(1, length + 1):
            ...
            ev = ObservedEvent(event_id, seq, ts.isoformat(), event_lifecycle_phase(et), et, [], attrs)
...
def _hard_negative_traps(accounts: list[str], rng: random.Random, n_each: int, start: datetime) -> list[dict[str, Any]]:
    ...
    for kind in kinds:
        for i in range(n_each):
            ...
            ev = ObservedEvent(f"syn_trap_{kind}_{i:06d}", 1, ts.isoformat(), event_lifecycle_phase(et), et, [], attrs)
