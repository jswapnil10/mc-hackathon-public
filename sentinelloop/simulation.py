...
def _materialize_attributes(
    ...
    # Fix A (reverse-leak): auth/confirmation fields must not be present ONLY in legit rows,
    # else a model learns "field present => safe". Stamp them on every value/session event so
    # attacks carry them too — mostly False under attack (an attacker rarely clears strong auth
    # or genuine confirmation), but occasionally True so a True value is not a guaranteed alibi.
    if event_type in VALUE_EVENT_TYPES or event_type == "SESSION_STARTED":
        materialized.setdefault("strong_authentication_passed", rng.random() < 0.2)
        materialized.setdefault("customer_confirmation_complete", rng.random() < 0.2)
    return materialized
