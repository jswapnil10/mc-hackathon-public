...
Reasoning is role-aware rather than globally disabled: Red planning and offline Blue strategy use medium effort, while the latency-sensitive Blue event path uses low effort. The gateway remains portable to ordinary non-reasoning models and older OpenAI-compatible servers: it retries without the optional `reasoning_effort` field when that field is rejected. If a reasoning model consumes its output budget without producing the required JSON, the same call is retried once with reasoning disabled. Both fallbacks are recorded in the run trace.
