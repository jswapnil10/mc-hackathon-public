...
The default is direct structured generation (`MODEL_REASONING_EFFORT=none`) for Red planning, live Blue decisions and offline Blue strategy. Red still returns explicit objectives, adaptation hypotheses, reasoning summaries and rationales; only the model's separate hidden thinking stream is disabled. Faster inference servers may opt into the advanced role-aware profile with `auto`. Optional reasoning receives only part of the end-to-end request window before the gateway retries direct JSON generation, and endpoints that reject the optional field are retried without it. These fallbacks are recorded in the run trace.

Live placement is intentionally two-speed. A pure deterministic calculation produces the minimum action before Qwen is called. Independently verified legitimate context may resolve an event immediately only when that minimum remains `allow` and there is no unresolved alert. All other events use one evidence-grounded Qwen response for both investigation and action, while isolated attack and look-alike cases may run concurrently. If Qwen misses its operational window, the event is explicitly marked as degraded and the observable guard plus Blue-only detector retain control using reversible, proportionate actions; model unavailability alone can never cause a hard block. Transaction events declare a 300 ms lane, pre-transaction events a 2 s lane, and post-transaction events a 5 s streaming lane. The prototype records actual model and end-to-end battle latency but does not pretend local Qwen is a 300 ms hard dependency: model investigation and explanation belong in the asynchronous control plane while the guardrail can sit inline.
