...
- Private source remote: `origin` (`https://github.com/u367403_ual/mc-hackathon.git`)
- Public/deployment remote: `public` (`https://github.com/jswapnil10/mc-hackathon-public.git`)
- Integrated UI source: local Downloads branch `codex/offline-replay-deployment` at `b920a11` (including `a53ba25` and `d4a0ef2`)

The branch now combines the public offline-replay deployment with the `new-ui-deployment` work. The integration preserves the public/offline replay disclosure, configurable repository link, recorded-model provenance, one-to-five-round catalog, short presentation pacing, **Show full report** escape hatch, and **Start battle** button while adding the dedicated Evidence workspace, live grouped event stream, money counterfactual, and revised Arena report.
...
- Offline data loading must remain immediate and reliable. Keep the short paced reveal (about 4 seconds for one round and 9 seconds for five) plus the **Show full report** escape hatch so the replay feels like a battle without pretending an LLM is running on the host.
...
The Downloads repository's integrated UI branch has been merged into `codex/offline-replay-deployment`. The result includes:
...
Do not run destructive Git cleanup commands. Do not overwrite user-owned presentation assets. Push or change repository visibility only with explicit user authorization. The approved public deployment target for this work is `https://github.com/jswapnil10/mc-hackathon-public`, with Render tracking its `main` branch.
