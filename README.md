...
The repository includes a ready-to-import [Render Blueprint](render.yaml) for a public, shareable demo URL. It serves the Identify, Generate, Defend and reproducible benchmark views without exposing a model credential. Its default `DEMO_MODE=precomputed` returns recorded synthetic Agent Arena replays from `data/demo_runs/`; it makes no model or external API call and labels results as precomputed replays. The catalogue covers all nine attack families at easy, medium and hard difficulty, with both one-round Qwen recordings and complete five-round adaptive recordings. The model that originated each artifact remains visible as provenance rather than being presented as the model currently running. The UI supports one to five instant recorded rounds for every selection and clearly distinguishes replay mode from a live agent battle.
