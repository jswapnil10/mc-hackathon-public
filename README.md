...
The repository includes a ready-to-import [Render Blueprint](render.yaml) for a public, shareable demo URL. It serves the Identify, Generate, Defend and reproducible benchmark views without exposing a model credential. Its default `DEMO_MODE=precomputed` returns recorded synthetic Agent Arena replays from `data/demo_runs/`; it makes no model or external API call and labels results as precomputed replays. The committed Qwen 3.5 9B catalog covers all nine attack families at easy, medium and hard difficulty. The UI supports one to five instant replay cycles for every selection. Until a curated adaptive multi-round artifact is added, extra cycles replay the available recorded snapshot and are explicitly labelled as non-adaptive rather than claiming live model learning.
