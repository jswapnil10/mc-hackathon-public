...
## Public hackathon deployment (Render)

The repository includes a ready-to-import [Render Blueprint](render.yaml) for a public, shareable demo URL. It serves the Arena and dedicated Identify, Generate, and Defend evidence workspace without exposing a model credential. Its default `DEMO_MODE=precomputed` returns recorded synthetic Agent Arena replays from `data/demo_runs/`; it makes no model or external API call and labels results as precomputed replays. The catalogue covers all nine attack families at easy, medium and hard difficulty, with both one-round Qwen recordings and complete five-round adaptive recordings. The model that originated each artifact remains visible as provenance rather than being presented as the model currently running. The UI supports one to five recorded rounds for every selection, reveals the stored plans, grouped payment events, Blue decisions, counterfactual money outcome, and Referee score with short presentation pacing, and offers a **Show full report** escape hatch.

1. Push this repository to GitHub.
2. In [Render](https://render.com/), choose **New + → Blueprint**, connect the GitHub repository and accept `render.yaml`.
3. Deploy. Render builds `deploy/Dockerfile`, trains the bundled synthetic-data detector during the image build, and gives the service a public `https://…onrender.com` URL.
4. Open `https://YOUR-SERVICE.onrender.com/healthz` to confirm `{"status":"ok"}`, then submit the root URL.

For a live Agent Battle, clear `DEMO_MODE` and add `MODEL_BASE_URL`, `MODEL_API_KEY`, `RED_MODEL_ID` and `BLUE_MODEL_ID` in Render's environment settings. The model server must not be publicly exposed; use a private network endpoint or authenticated gateway. The browser never receives these settings. Do not add a real `.env` file or an API key to the repository.

The replay disclosure links back to this setup guide. If the project is moved to another repository, set `SOURCE_REPOSITORY_URL` to that repository's public URL so the in-product **Connect your own open model** link stays correct.

The free Render plan can sleep while idle, so the first page load may take a short time. For a smooth judging demo, open the URL shortly before presenting or use an always-on plan.
...
$env:MODEL_REASONING_EFFORT = "none"
python -m sentinelloop config
python -m app.server
```

Direct structured generation is the reliable local Ollama profile. Red still generates an
objective, adaptation hypothesis, reasoning summary, rationales and bounded parameter changes;
the `none` profile disables only the separate hidden reasoning stream that can consume the whole
request window before JSON is produced. Faster hosted inference can opt into advanced role-aware
